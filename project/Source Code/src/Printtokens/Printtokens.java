...
//	   System.out.println("res "+res+" id "+id+" ch "+ch+" sb "+sb.toString());
	   if(res == -1 && id != 2)   //&& id != 2 added    /* if end character is eof token    */
	   ...
	   if(id==2)       //id == 1           /* if end character is " and is string */
	     {
		   if (ch == '"') {
			   sb.append(ch);
		   }
	   ...
	   return sb.toString();                   /* return normal case token             */
	}
	...
	static boolean is_token_end(int str_com_id, int res)
	{
	 ...
	 char ch = (char)res;
	 if(str_com_id==2)     //changed from 1     /* is string token */
	 ...
	 if(str_com_id==1)   //changed from 2 /* is comment token */
	...
	{
	   if (str.length() == 2 && str.charAt(0)=='#' && Character.isLetter(str.charAt(1))) // changed str.length() > 2 to < 3
	     return true;
	  else  
	     return false; 
	}
	...
	static boolean is_num_constant(String str)
	{
	  ...
	  if ( Character.isDigit(str.charAt(0))) 
	    {
		  // while ( i <= str.length() && str.charAt(i) != '\0' ) to line 306
	...
	static boolean is_str_constant(String str)
	{
	  ...
	  if ( str.charAt(0) =='"')
	     { while (i < str.length() && str.charAt(i)!='\0')  /* until meet the token end sign */ //charAt(0) to charAt(i)
	         { if(str.charAt(i)=='"')
	             return true;        /* meet the second '"'           */
	           else
	           i++;
	         }               /* end WHILE */
	     return false;	//return true to return false
	    }
	  else
	    return false;       /* other return FALSE */
	}
	  ...
	  if ( Character.isLetter(str.charAt(0)) ) 
	     {
		    ...
		    return true; // return false to return true 
	     }
	  else
		return false; //return true to return false
	}
